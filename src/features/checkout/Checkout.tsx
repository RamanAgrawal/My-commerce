...
const Checkout = () => {
  ...
  return (
    <>
      {" "}
      {!cart.length &&
        (alert("please add something first"), (<Navigate to={"/"} />))}
      {currentOrder &&
        (currentOrder.PaymentMethod === "cash" ? (
          <Navigate to={`/order-succcess/${currentOrder.id}`} replace={true} />
        ) : (
            ...
            {user && user.addresses.length > 0 && (
              <>
                ...
                <ul role="list" className="divide-y divide-gray-100">
                  {user &&
                    user.addresses.map((address: AddressI, index) => (
                      <li
                        key={index}
                        className="flex justify-between gap-x-6 py-3 border-solid border-4 mb-3 p-2"
                      >
