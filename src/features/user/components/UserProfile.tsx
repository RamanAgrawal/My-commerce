...
const UserProfile: FC = () => {
  ...
  const handleDelete = async () => {
    if (userInfo) {
      const newItem = userInfo?.addresses.filter((_, i) => i !== deleteIndex);
      await dispatch(
        updateUserAsync({ ...userInfo, addresses: newItem || [] })
      );
      setOpenModel(false);
    }
  };
  return (
    <div className="mx-auto max-w-6xl px-2 sm:px-6 lg:px-8 bg-white 4 pt-44">
      {openModel && (
        <Model text={"address"} setOpenModel={setOpenModel} handleDelete={handleDelete} />
      )}
      <EditAddressModal
        editAddressData={address}
        open={openEditModal}
        setOpen={setEditOpenModal}
        handleSubmitHandler={handleEdit}
      />
      <div className="border-t border-gray-200 px-4 py-6 sm:px-6">
        <div className="flow-root">
          ...
          <h3 className="text-2xl font-bold mb-2">{userInfo?.email}</h3>
        </div>
      </div>
      ...
      <div className="border-t border-gray-200 px-4 py-6 sm:px-6">
        ...
        <div className="grid lg:grid-cols-3 grid-cols-1 gap-5">
          <Link
            to={"/addaddress"}
            className=" self-stretch flex justify-center mt-3 items-center p-4 border-dotted border-2 border-gray-400 h-60 rounded-md"
          >
          ...
          {userInfo?.addresses?.map((address, index) => (
            <div
              key={index}
              className="p-4 mt-3 self-stretch border-2 relative border-gray-400 h-60 box-border rounded-md"
            >
